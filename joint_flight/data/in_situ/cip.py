...
x = np.concatenate([cip_15["x"][start_15 : end_15],
                    cip_100["x"][start_100 : end_100]])
y  = np.concatenate([cip_15["dndd"][start_15 : end_15],
...
bins = np.concatenate([np.linspace(0, 1e-2, 6),
...
for i in range(y.shape[1] // n_avg):
    ...
    y_r[:, i], _ = np.histogram(xx[:, start:end].ravel(),
                                weights = y[:, start:end].ravel(), bins = bins)
