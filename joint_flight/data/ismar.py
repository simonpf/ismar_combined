...
faam_ismar = Dataset(
    glob.glob(os.path.join(data_path, "metoffice-ismar_faam*.nc"))[0], "r"
)
...
channel_names = [
    "".join([np.bytes_(c).decode() for c in cs])
    for cs in faam_ismar.variables["channel"]
]
...
nedt = faam_ismar.variables["brightness_temperature_random_error"][i_start:i_end, :][
    indices
]
