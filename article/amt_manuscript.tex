...
\begin{abstract}
  ...
  to validate the retrieval implementation and asses the capability of the
  retrieval to constrain the properties ice hydrometeors. Good agreement with
  in-situ measurements of ice water content and particle concentrations for
  particle maximum diameters $> 200\ \unit{\mu m}$ is found for one of the
  flights. The variational retrieval fits the observations well for most
  channels, which serves as a validation radiative transfer modeling of cloudy
  atmospheres at sub-millimeter wavelengths. Moreover, we find that the quality
  of the fit to the observations is independent of the assumed ice particle
  shape, indicating that the employed combination of observations is
  ...
  to the microphysical properties of the ice hydrometeors compared to a
  ...
  to better constrain their distribution especially in the lower parts of the
  cloud. Applied to airborne observations from field campaigns or as part of a
  future satellite mission, these type of retrievals could help to improve
  measurements of ice-hydrometeors. The good fits obtained to the observations
  further increase confidence in the modeling of clouds in the Atmospheric
  Radiative Transfer Simulator and the corresponding single scattering database,
  which were used to implement the retrieval forward model. Our results
  demonstrate the suitability of these tools to produce realistic simulations
  for upcoming sub-millimeter sensors such as the Ice Cloud Image or the Arctic
  Weather Satellite.
...
and potential of synergistic retrievals of ice hydrometeors combining active and
passive observations at millimeter and sub-millimeter wavelengths. Since the
combined retrieval can better constrain the PSD of ice hydrometeors, it may be
used to study the representation of clouds in NWP and climate models.
Additionally, as illustrated in this study, the retrieval can be used as a tool
to study the representation of ice hydrometeors in radiative transfer
simulations, which will be vital to many applications of observations from
upcoming sub-millimeter sensors such as ICI and the Arctic Weather Satellite.
