...
%DIF DEL amt_manuscript.tex           Wed Dec 15 20:35:40 2021
%DIF ADD amt_manuscript_revised.tex   Wed Dec 22 16:48:35 2021
%% ---------------------------------
...
\begin{abstract}
  ...
  to validate the retrieval and \DIFdelbegin \DIFdel{asses }\DIFdelend \DIFaddbegin \DIFadd{assess }\DIFaddend its capabilities to constrain the
  properties of ice hydrometeors. To account for uncertainty in the assumed
  shapes of ice particles, the retrieval is run multiple times while the shape
  is varied. Good agreement with in situ measurements of ice water content and
  particle concentrations for particle maximum diameters larger than
  ...
  of the sub-millimeter channels pointing towards issues with the sensor
  calibration or the modeling of gas absorption. \DIFdelbegin \DIFdel{We find that }\DIFdelend \DIFaddbegin \DIFadd{For one of the flights }\DIFaddend the
  quality of the fit to the observations \DIFdelbegin \DIFdel{is independent of }\DIFdelend \DIFaddbegin \DIFadd{exhibits a weak dependency on }\DIFaddend the
  assumed ice particle shape, indicating that the employed combination of
  observations \DIFdelbegin \DIFdel{is insufficient to
  constrain }\DIFdelend \DIFaddbegin \DIFadd{may provide limited information on }\DIFaddend the shape of ice particles in
  the observed clouds. Compared to a radar-only retrieval, the results show an
  improved sensitivity of the synergistic retrieval to the microphysical
  properties of ice hydrometeors at the base of the cloud.

  Our findings indicate that the synergy between active and passive microwave
  observations \DIFaddbegin \DIFadd{may }\DIFaddend improve remote-sensing measurements of ice hydrometeors and
  \DIFdelbegin \DIFdel{may
  }\DIFdelend thus help to reduce uncertainties that affect currently available data
...
designated B984, \DIFdelbegin \DIFdel{took place }\DIFdelend \DIFaddbegin \DIFadd{was performed }\DIFaddend on 14 October 2016 as part of the North Atlantic
Waveguide and Downstream Impact Experiment (NAWDEX\DIFdelbegin \DIFdel{, \mbox{%DIFAUXCMD
\citet{schafler18}}\hspace{0pt}%DIFAUXCMD
)}\DIFdelend \DIFaddbegin \DIFadd{), which took place during
September and October 2016 \mbox{%DIFAUXCMD
\citep{schafler18}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . During this flight, a cloud
...
(SAFIRE). The two other flights, designated C159 and C161, \DIFdelbegin \DIFdel{took
place in March 2019 as }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend part of the
PIKNMIX-F campaign\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, which took place in March 2019. }\DIFaddend These two flights were
...
\DIFaddbegin \DIFadd{While the radar observations for flight B984 come from an airborne radar, the
...
\begin{figure}[hbpt!]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/fig17}
  \caption{\DIFaddFL{Delays between the co-located observations from radar and radiometers
  for the three flights.}}
  \label{fig:delay}
\end{figure}

\DIFaddend \subsubsection{MARSS}
...
\DIFaddbegin \DIFadd{The }\DIFaddend in situ measurements \DIFdelbegin \DIFdel{of cloud hydrometeors were performed during the }\DIFdelend \DIFaddbegin \DIFadd{that are relevant to this study are measurements of
bulk ice water content using a Nevzorov hot-wire probe \mbox{%DIFAUXCMD
\citep{korolev13} }\hspace{0pt}%DIFAUXCMD
and
...
respectively. In situ measurements are available only for }\DIFaddend flights B984 and C159\DIFaddbegin \DIFadd{,
...
cloud to perform the in situ measurements}\DIFaddend . A detailed view of the \DIFdelbegin \DIFdel{high-level runs and the corresponding
in situ sampling paths }\DIFdelend \DIFaddbegin \DIFadd{high and low
level runs for the two flights }\DIFaddend are provided in
Fig.~\ref{fig:flight_overview_detail}. For flight C159, this view reveals a
noticeable horizontal offset of $3$ to $4\ \unit{km}$ between the ground \DIFdelbegin \DIFdel{track }\DIFdelend \DIFaddbegin \DIFadd{tracks
}\DIFaddend of radar and radiometer observations\DIFdelbegin \DIFdel{and
even larger deviations between the lower }\DIFdelend \DIFaddbegin \DIFadd{. Even larger deviations occur between
certain }\DIFaddend parts of the \DIFdelbegin \DIFdel{in situ sampling flight
path and the high-level run}\DIFdelend \DIFaddbegin \DIFadd{low level run and the ground tracks of the remote sensing
...
\DIFdelbegin \DIFdel{The in situ measurements that are relevant to this study are bulk ice water
content measured using a Nevzorov hot-wire probe \mbox{%DIFAUXCMD
\citep{korolev13} }\hspace{0pt}%DIFAUXCMD
and PSDs
...
spatially closest radiometer observations. Background properties of \DIFdelbegin \DIFdel{atmosphere
and }\DIFdelend \DIFaddbegin \DIFadd{the
atmosphere and the }\DIFaddend surface, such as temperature and wind speed, as well as a
...
water content (LCWC) and relative humidity. \DIFdelbegin \DIFdel{All }\DIFdelend \DIFaddbegin \DIFadd{Hydrometeor PSDs are represented
using the approach proposed by \mbox{%DIFAUXCMD
\citet{delanoe05}}\hspace{0pt}%DIFAUXCMD
...
}\begin{align}\DIFadd{\label{eq:psd}
  N(D_\text{eq}) }&\DIFadd{= N_0^{*}F(\frac{D_\text{eq}}{D_m})
}\end{align}
\DIFadd{where $F$ is a fixed function that specifies the shape of the normalized PSD and
...
of all }\DIFaddend retrieval targets and corresponding a priori assumptions \DIFdelbegin \DIFdel{are listed }\DIFdelend \DIFaddbegin \DIFadd{is provided }\DIFaddend in
...
\citet{cazenave19}}\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL .}
 \centering
\label{tab:a_priori}
    ...
    }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{500\ }%DIFDELCMD < \unit{\mu m} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$500\ \unit{\mu m}$ }\DIFaddendFL \\
    ...
    & $D_m$
    & \DIFdelbeginFL \DIFdelFL{500\ }%DIFDELCMD < \unit{\mu m}
%DIFDELCMD <     %%%
...
The \DIFdelbegin \DIFdel{retrieval forward model has been }\DIFdelend \DIFaddbegin \DIFadd{forward model and retrieval were made adaptive so that the ingested
observations can be easily }\DIFaddend adapted to the \DIFdelbegin \DIFdel{sensors }\DIFdelend \DIFaddbegin \DIFadd{different sensors and channels }\DIFaddend that
were available for each flight. \DIFaddbegin \DIFadd{Low frequency channels that are used only over
Ocean surfaces are deactivated over land by setting the corresponding channel
uncertainty to $10^6\ \unit{K}$. }\DIFaddend The atmospheric grid was limited to altitudes
between $0$ and $10\ \unit{km}$ and matched to the resolution of the radar
observations. The latest stable release (version 2.4) of \DIFdelbegin \DIFdel{the Atmospheric Radiative Transfer
Simulator (ARTS , \mbox{%DIFAUXCMD
\citet{arts18}}\hspace{0pt}%DIFAUXCMD
) }\DIFdelend \DIFaddbegin \DIFadd{ARTS \mbox{%DIFAUXCMD
\citep{arts18} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend is
...
from water \DIFdelbegin \DIFdel{varpor }\DIFdelend \DIFaddbegin \DIFadd{vapor }\DIFaddend is calculated using a combination of the AER database v3.6
...
properties. \DIFdelbegin \DIFdel{More specifically}\DIFdelend \DIFaddbegin \DIFadd{As described above}\DIFaddend , the forward model uses the normalized PSD
approach proposed by \citet{delanoe05} with the mass-weighted mean diameter
($D_m$) and intercept parameter ($N_0^*$) as parameters. The \DIFdelbegin \DIFdel{updated values from
\mbox{%DIFAUXCMD
\citet{cazenave19} }\hspace{0pt}%DIFAUXCMD
are used as shape
parameters of the distribution }\DIFdelend \DIFaddbegin \DIFadd{normalized shape
function $F$ in Eq.~(\ref{eq:psd}) follows a modified gamma distribution shape
using the parameters from \mbox{%DIFAUXCMD
\citet{cazenave19}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The ice particle habit is
represented by a collection of ice particle shapes \DIFdelbegin \DIFdel{together
with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend corresponding,
pre-computed \DIFaddbegin \DIFadd{single particle }\DIFaddend optical properties. Bulk optical properties are
...
cannot be retrieved and must be assumed a priori. \DIFdelbegin \DIFdel{Since this is difficult, }\DIFdelend \DIFaddbegin \DIFadd{Due to the large variability
...
best model exists at all. Hence, the approach taken here is to select }\DIFaddend a set of
habits \DIFdelbegin \DIFdel{has been chosen with which the retrieval will be run in order }\DIFdelend \DIFaddbegin \DIFadd{and perform the retrieval with each of them. This will allow us }\DIFaddend to
investigate the impact of the selected habit on the \DIFaddbegin \DIFadd{retrieval }\DIFaddend results.
...
Radiometer residuals for flight B984 are mostly within $\pm 5\ \unit{K}$ \DIFdelbegin \DIFdel{. For
the two other flightsthe residuals are larger. Differences up to and }\DIFdelend \DIFaddbegin \DIFadd{but
larger for flights C159 and C161. For these two flights, residuals }\DIFaddend exceeding
$10\ \unit{K}$ are observed \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the window channels up to $243\ \unit{GHz}$ as
well as \DIFdelbegin \DIFdel{in }\DIFdelend the outermost channels around the absorption lines at $118\ \unit{GHz}$
and $183\ \unit{GHz}$. Since these \DIFdelbegin \DIFdel{correspond to profiles in
which residuals of opposite sign are present }\DIFdelend \DIFaddbegin \DIFadd{occur in profiles where precipitation is
present and in which similar residuals can be observed }\DIFaddend in the radar observations
\DIFaddbegin \DIFadd{and other channels that are sensitive to the lower parts of the atmosphere}\DIFaddend , a
likely explanation is that they are caused by \DIFdelbegin \DIFdel{small-scale precipitation events that are
missed by one of the }\DIFdelend \DIFaddbegin \DIFadd{precipitation that is not observed
by all }\DIFaddend sensors due to \DIFaddbegin \DIFadd{spatial and temporal }\DIFaddend co-location issues. \DIFaddbegin \DIFadd{Especially the
...
can fit the observations \DIFaddbegin \DIFadd{fairly well }\DIFaddend regardless of the assumed ice particle
...
in the retrieval residuals, \DIFaddbegin \DIFadd{we have investigated the relationship between
...
\unit{GHz}$ channel in }\DIFaddend Fig.~\ref{fig:residuals_scatter} in the appendix\DIFdelbegin \DIFdel{displays the relation }\DIFdelend \DIFaddbegin \DIFadd{. We did
...
Aggregate seem to manifest a positive correlation }\DIFaddend between IWP and \DIFdelbegin \DIFdel{corresponding residualsin }\DIFdelend \DIFaddbegin \DIFadd{the residuals.
...
least for these two channels }\DIFaddend the \DIFdelbegin \DIFdel{$325 \pm
3.5 \unit{GHz}$ channel. This channel was chosen because it
belongs to the channels displaying the largest differences between residual distributions for
different particle habits (}\DIFdelend \DIFaddbegin \DIFadd{Large Plate Aggregate seems to stand out as the
ice particle shape yielding the smallest residuals across the retrieved range of
IWP values.
}

\DIFadd{Since the Large Plate Aggregate is one of the particles for which the best
...
results from }\DIFaddend Fig.~\DIFdelbegin \DIFdel{\ref{fig:residuals_box})
. Nonetheless, the
plots exhibit no sign of a relationship between either residual and ice
water content or the residuals across different habits}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:residuals_scatter_b984}, it remains unclear whether
...
\DIFdelbegin \DIFdel{This result }\DIFdelend \DIFaddbegin \begin{figure}[!hbpt]
  \centering
  \includegraphics[width=1.0\textwidth]{figures/fig18}
  \caption{
    \DIFaddFL{Brightness temperature residuals between true and simulated observations for
...
%DIF > Another interesting aspect of these results is the selection of channels in
%DIF > which indications of a signal from the particle shape are present: These are
%DIF > channels with a comparably high atmospheric transmission, which means that they
%DIF > are sensitive to the lower parts of the cloud. Our interpretation is that this
%DIF > is likely due to the structure of the observed clouds, which have sufficiently
%DIF > large ice particles for their shape to have a significant impact of the passive
%DIF > observations only at their base. There may thus be a difference in which
%DIF > channels contribute to the synergy between radar and passive sub-millimeter
%DIF > observations for mid-latitude cloud systems compared to deep convective cloud
%DIF > systems more likely to be found in the (sub-)tropics. While deep convective
%DIF > systems may have sufficiently large particles aloft for a significant signal in
%DIF > the frequencies 

\DIFadd{Nonetheless, even if indeed present, a potential signal from the ice particle
shape in the results would be limited to a few Kelvin. This }\DIFaddend implies that future
...
\DIFdelbegin \DIFdel{consistency }\DIFdelend \DIFaddbegin \DIFadd{agreement up to a few Kelvin }\DIFaddend between simulations and observations can be
...
The lack of a \DIFaddbegin \DIFadd{distinct }\DIFaddend signal that constrains the ice particle shape even in the
...
Plate Aggregate and the 6-Bullet Rosette. \DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{The channel residuals presented in
...
}\DIFaddend is interesting to note that the Large Plate Aggregate was also found to yield
...
\DIFdelbegin \DIFdel{observed deviations are }\DIFdelend \DIFaddbegin \DIFadd{concentrations of these larger particles may be retrieved correctly but that the
total IWC is underestimated }\DIFaddend due to the \DIFdelbegin \DIFdel{presence of a large number of small ice
particles
that the microwave observations are not sensitive to}\DIFdelend \DIFaddbegin \DIFadd{mismatch between assumed and actual PSD
shape, the former of which lacks the very high concentration of small particles
that are present in the in situ measurements}\DIFaddend . Although \citet{oshea21} and
\citet{oshea19} show that the \DIFdelbegin \DIFdel{occurence }\DIFdelend \DIFaddbegin \DIFadd{occurrence }\DIFaddend of high particle concentrations at
...
\begin{figure}[!hbpt]
  ...
  \caption{\DIFdelbeginFL \DIFdelFL{in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{In }\DIFaddendFL situ measured and retrieved PSDs for flight B984
...
\DIFaddbegin \DIFadd{The tendencies observed for the retrieved IWC in Fig.~14 are even more
...
\begin{table}
  \centering
  \caption{\DIFaddFL{Retrieved IWP along in situ flight path for flight B984 for the
...
\begin{tabular}{l|rr}
  ...
  \hline
  \hline
\textbf{\DIFaddFL{In situ}} & \multicolumn{2}{c}{$0.3615 $} \\
\hline
 \textbf{\DIFaddFL{Habit}} & \DIFaddFL{Combined }& \DIFaddFL{Radar-only }\\
\DIFaddFL{6-Bullet Rosette }& \DIFaddFL{$0.3362  $ }& \DIFaddFL{$0.4971  $ }\\ 
\DIFaddFL{8-Column Aggregate }& \DIFaddFL{$0.2383  $ }& \DIFaddFL{$0.6783  $ }\\ 
\DIFaddFL{Large Column Aggregate }& \DIFaddFL{$0.7868  $ }& \DIFaddFL{$0.4116  $ }\\ 
\DIFaddFL{Large Plate Aggregate }& \DIFaddFL{$0.3666  $ }& \DIFaddFL{$0.4664  $ }\\ 
\DIFaddFL{Evans Snow Aggregate }& \DIFaddFL{$0.7082  $ }& \DIFaddFL{$0.5073  $ }\\ 
\end{tabular}
\end{table}
...
\DIFaddbegin \DIFadd{While these results were obtained for a Ka band cloud radar, we do not expect
...
particle habit that is used in the retrieval forward model. \DIFdelbegin \DIFdel{We found
no }\DIFdelend \DIFaddbegin \DIFadd{Although we found
some }\DIFaddend evidence of a signal that could help to constrain the ice particle shape
based \DIFdelbegin \DIFdel{solely }\DIFdelend on the combination of radar and \DIFdelbegin \DIFdel{microwave observations, not even when
sub-millimeter observations are included}\DIFdelend \DIFaddbegin \DIFadd{sub-millimeter observations, it remains
limited to not more than $5$ Kelvin}\DIFaddend . This means that more work is needed
to \DIFdelbegin \DIFdel{better constrain the shape a priori or that even more observations must be
integrated into the retrieval}\DIFdelend \DIFaddbegin \DIFadd{find out how to effectively constrain the ice particle shape with
remote sensing observations or to better constrain it a priori}\DIFaddend .
...
\begin{figure}
  ...
  \caption{
    ...
    observations \DIFaddbeginFL \DIFaddFL{for the }\DIFaddendFL $325 \pm 3.5 \unit{GHz}$ ISMAR channel. Each column displays
    the residual distributions for the five different particle habits\DIFaddbeginFL \DIFaddFL{. The gray
    ...
    of a two sided significance test for the slope of regression line}\DIFaddendFL .
  }
