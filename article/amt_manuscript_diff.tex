...
%DIF DEL amt_manuscript.tex           Wed Dec 15 18:20:50 2021
%DIF ADD amt_manuscript_revised.tex   Wed Dec 15 18:06:55 2021
%% ---------------------------------
...
\begin{abstract}
  ...
  \DIFdelbegin \DIFdel{assumed
  }\DIFdelend shape is varied. Good agreement with in situ measurements of ice water content
  ...
  of the sub-millimeter \DIFaddbegin \DIFadd{channels }\DIFaddend pointing towards issues with the sensor calibration or
...
is provided in Fig.~\ref{fig:flight_overview}. The first considered flight, \DIFdelbegin \DIFdel{designated B984, took place on }\DIFdelend \DIFaddbegin \DIFadd{from
}\DIFaddend 14 October 2016 \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{and designated as B984, was }\DIFaddend part of the North Atlantic Waveguide
and Downstream Impact Experiment (NAWDEX\DIFdelbegin \DIFdel{, \mbox{%DIFAUXCMD
\citet{schafler18}}\hspace{0pt}%DIFAUXCMD
)}\DIFdelend \DIFaddbegin \DIFadd{), which took place during September and
October 2016 \mbox{%DIFAUXCMD
\citep{schaefler18}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . During this flight, a cloud system generated
...
The two other flights, designated C159 and C161, \DIFdelbegin \DIFdel{took
place in March 2019 as }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend part of the PIKNMIX-F
campaign \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{which took place in March 2019. }\DIFaddend These two flights were performed
...
\DIFaddbegin \DIFadd{The }\DIFaddend in situ measurements \DIFaddbegin \DIFadd{that are relevant to this study are bulk ice water
...
\DIFadd{In situ measurements }\DIFaddend of cloud hydrometeors were performed during the flights
B984 and C159. \DIFaddbegin \DIFadd{These flights consist of two parts: A high-level run during which
...
descended through the cloud to perform the in situ measurements. }\DIFaddend A detailed view
of the \DIFdelbegin \DIFdel{high-level runs and the corresponding
in situ sampling paths }\DIFdelend \DIFaddbegin \DIFadd{high- and low-level runs for the two flights }\DIFaddend are provided in
...
%DIFDELCMD < %%%
\DIFdelend An overview of the \DIFaddbegin \DIFadd{in situ }\DIFaddend measured IWC and PSDs is provided in Fig.~\ref{fig:in_situ}.
...
and \DIFaddbegin \DIFadd{the }\DIFaddend surface, such as temperature and wind speed, as well as a priori profiles
...
(LCWC) and relative humidity. \DIFaddbegin \DIFadd{Concentrations of hydrometeors are represented
using the normalized PSD approach proposed by \mbox{%DIFAUXCMD
...
volume equivalent $D_\text{eq}$ is given by:
  ...
  \DIFadd{N(D_\text{eq}) }&\DIFadd{= N_0^{*}F(\frac{D_\text{eq}}{D_m})
}\end{align}
\DIFadd{Here $F$ is a fixed function that specifies the shape of the normalized PSD. The
...
using a $\text{log}$ transform. }\DIFaddend All retrieval targets and corresponding a priori
...
to $10^6\ \unit{K}$. }\DIFaddend The atmospheric grid was limited to altitudes between $0$
and $10\ \unit{km}$ and matched to the resolution of the radar observations. The
latest stable release (version 2.4) of \DIFdelbegin \DIFdel{the Atmospheric Radiative Transfer
Simulator (ARTS , \mbox{%DIFAUXCMD
\citet{arts18}}\hspace{0pt}%DIFAUXCMD
) }\DIFdelend \DIFaddbegin \DIFadd{ARTS \mbox{%DIFAUXCMD
\citep{buehler18} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend is used to implement the forward model used in
...
particle habit is represented by a collection of ice particle shapes \DIFdelbegin \DIFdel{together
with }\DIFdelend \DIFaddbegin \DIFadd{and
}\DIFaddend corresponding, pre-computed \DIFaddbegin \DIFadd{single particle }\DIFaddend optical properties. Bulk optical
...
the particle habit cannot be retrieved and must be assumed a priori. \DIFdelbegin \DIFdel{Since this is difficult, }\DIFdelend \DIFaddbegin \DIFadd{Due to the
...
therefore to select }\DIFaddend a set of habits \DIFdelbegin \DIFdel{has been chosen with which the retrieval will be run in order }\DIFdelend \DIFaddbegin \DIFadd{and perform the retrieval with each of them.
This will allow us }\DIFaddend to investigate the impact of the selected habit on the
results.
...
the two other flights the residuals are larger. \DIFdelbegin \DIFdel{Differences up to and }\DIFdelend \DIFaddbegin \DIFadd{For the other two flight,
differences }\DIFaddend exceeding $10\ \unit{K}$ are observed \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the window channels up to
$243\ \unit{GHz}$ as well as \DIFdelbegin \DIFdel{in }\DIFdelend the outermost channels around the absorption lines
at $118\ \unit{GHz}$ and $183\ \unit{GHz}$. Since these correspond to profiles
in which \DIFdelbegin \DIFdel{residuals of opposite sign are present }\DIFdelend \DIFaddbegin \DIFadd{similar residuals occur }\DIFaddend in the radar observations, a likely explanation
is that they are caused by \DIFdelbegin \DIFdel{small-scale precipitation events that are
missed by one of the }\DIFdelend \DIFaddbegin \DIFadd{liquid precipitation that is not observed by all }\DIFaddend sensors
...
retrieval using real observations remained an open issue. For flight \DIFdelbegin \DIFdel{, }\DIFdelend B984 good
...
\DIFdelbegin \DIFdel{observed deviations are due to the presence of a large number of small }\DIFdelend \DIFaddbegin \DIFadd{concentrations of }\DIFaddend ice particles that the \DIFdelbegin \DIFdel{microwave observations are not sensitive to }\DIFdelend \DIFaddbegin \DIFadd{sensors are sensitive to are retrieved
...
actual PSD shape}\DIFaddend . Although \citet{oshea21} and \citet{oshea19} show that the
\DIFdelbegin \DIFdel{occurence }\DIFdelend \DIFaddbegin \DIFadd{occurrence }\DIFaddend of high particle concentrations at sizes below $200\ \unit{\mu m}$
...
(Fig.~\ref{fig:mass_size_relation}). \DIFaddbegin \DIFadd{This means that the PSD measurements are
consistent with the IWC measurements across a wide range of realistic mass-size
relationships. }\DIFaddend Furthermore, the presence of a cloud layer with a large number of
